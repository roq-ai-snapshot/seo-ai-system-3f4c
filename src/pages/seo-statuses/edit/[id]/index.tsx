...
import { getSeoStatusById, updateSeoStatusById } from 'apiSdk/seo-statuses';
...
function SeoStatusEditPage() {
  ...
  const handleSubmit = async (values: SeoStatusInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSeoStatusById(id, values);
      mutate(updated);
      resetForm();
