...
import { getSeoStatusById } from 'apiSdk/seo-statuses';
import { SeoStatusInterface } from 'interfaces/seo-status';

function SeoStatusViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<SeoStatusInterface>(
    () => (id ? `/seo-statuses/${id}` : null),
    () =>
      getSeoStatusById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('seo_status', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/seo-statuses/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Backlinks" text={data?.backlinks} />

                <FormListItem label="Traffic" text={data?.traffic} />
