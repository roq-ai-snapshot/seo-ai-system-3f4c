...
import { getSeoStatuses } from 'apiSdk/seo-statuses';
import { SeoStatusInterface } from 'interfaces/seo-status';
import { getSeoStrategies } from 'apiSdk/seo-strategies';
import { SeoStrategyInterface } from 'interfaces/seo-strategy';

import { SeoStrategyListPage as ShowCaseList } from 'pages/seo-strategies';
...
function DashboardPage() {
  ...
  const seoStatusStat = useSWR<PaginatedInterface<SeoStatusInterface>>(
    () => `/seo-statuses`,
    () => getSeoStatuses({ limit: 1, offset: 0 }),
  );

  const seoStrategyStat = useSWR<PaginatedInterface<SeoStrategyInterface>>(
    () => `/seo-strategies`,
    () => getSeoStrategies({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Seo Statuses',
      result: seoStatusStat?.data?.totalCount,
      isLoading: seoStatusStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Seo Strategies',
      result: seoStrategyStat?.data?.totalCount,
      isLoading: seoStrategyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`seo-strategies`} passHref legacyBehavior>
                  <Button
